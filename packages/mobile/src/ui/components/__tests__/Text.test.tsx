...
describe("Text", () => {
  ...
  describe("numeric stat variants (STORY-002 AC 2.3)", () => {
    ...
    it.each(["stat-md", "stat-lg", "stat-xl"] as const)(
      "%s renders in the numeric family with tabular figures",
      (variant) => {
        const { getByText } = renderWithTheme(
        ...
        // Divvy Up renders numerics in Hanken Grotesk with tabular-nums
        // (mirrors the web app's `.dd-num` treatment) so updating numbers
        // don't shift horizontally; there is no separate mono face.
        expect(JSON.stringify(node.props)).toContain("tabular-nums");
        expect(style.fontFamily ?? "").toMatch(/Hanken/i);
      },
    );
