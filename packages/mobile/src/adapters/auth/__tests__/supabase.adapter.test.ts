...
jest.mock("expo-linking", () => ({
  createURL: jest.fn((path: string) => `divvyup://${path}`),
}));
  ...
  describe("signInWithOAuth", () => {
    it("returns session after successful OAuth flow with hash params", async () => {
      ...
      (WebBrowser.openAuthSessionAsync as jest.Mock).mockResolvedValue({
        type: "success",
        url: "divvyup://auth/callback#access_token=oauth-access&refresh_token=oauth-refresh",
      });
    ...
    it("extracts tokens from query params as fallback", async () => {
      ...
      (WebBrowser.openAuthSessionAsync as jest.Mock).mockResolvedValue({
        type: "success",
        url: "divvyup://auth/callback?access_token=query-access&refresh_token=query-refresh",
      });
