...
import { receiptErrorHandler } from "../errorHandler";
...
function buildApp() {
  return new Elysia()
    .use(receiptErrorHandler)
    .post("/validate", () => "ok", {
      body: t.Object({ name: t.String() }),
    })
    .get("/not-found", () => {
      throw new AppError("NOT_FOUND", 404, "Receipt abc not found");
    })
    .post("/parse", () => "ok", {
      body: t.Object({ name: t.String() }),
    })
...
describe("errorHandler (other-service)", () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  });
  ...
  it("maps a thrown AppError('NOT_FOUND', 404, ...) to 404", async () => {
    ...
    expect(body.detail).toBe("Receipt abc not found");
  });
