...
/**
 * Global Elysia error handler for `core`. Registered first in the app chain so every thrown
 * error (validation, not-found, parse, `AppError`, or anything unrecognised) resolves to a
 * consistent JSON body and a single `[api:error]` log line.
 */
